...
interface DispatchProps {
    ...
    refreshOrders: () => any;
    notifyBuySellMarket: (amount: BigNumber, token: Token, side: OrderSide, tx: Promise<any>, txHash: string) => any;
}
...
class BuySellTokenStep extends React.Component<Props, State> {
    ...
    private readonly _confirmOnMetamasBuyOrSell = async () => {
        ...
        try {
            ...
            this.props.notifyBuySellMarket(amount, token, side, Promise.resolve(), fillOrdersTxHash);
...
const BuySellTokenStepContainer = connect(
    ...
    (dispatch: any) => {
        return {
            ...
            notifyBuySellMarket: (amount: BigNumber, token: Token, side: OrderSide, tx: Promise<any>, txHash: string) =>
                dispatch(addMarketBuySellNotification(amount, token, side, tx, txHash)),
